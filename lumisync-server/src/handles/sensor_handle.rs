...
pub async fn get_sensor_data(
    ...
    let initial_timestamp = range
        .start
        .unwrap_or(OffsetDateTime::now_utc() - Duration::from_secs(24 * 60 * 60));
    let last_timestamp = Arc::new(Mutex::new(initial_timestamp));

    let stream = wrappers::IntervalStream::new(interval(Duration::from_secs(3))).then(move |_| {
